...
namespace J_Hooking
{
    public partial class GlobalHotKeyForm : Form
    {
        ...
        public GlobalHotKeyForm()
        {
            ...
            // 핫키 해지 (폼 종료 중 ... 이벤트)
            FormClosing += (s, e) => UnregisterHotKey(Handle, 0);
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_HOTKEY)
            {
                #region 이해를 돕기 위한 참조코드
                // 핫키에서 키값 비트연산
                // Modifier Key
                int x = (int)m.LParam;// & 0xFFFF;
                label1.Text += $"(B1) {Convert.ToString(x, toBase: 2).PadLeft(32, '0')}\n";
                int x_f = 0xFFFF;
                label1.Text += $"(Bf) {Convert.ToString(x_f, toBase: 2).PadLeft(32, '0')}\n";
                int x_2 = (int)m.LParam & 0xFFFF;
                label1.Text += $"(B2) {Convert.ToString(x_2, toBase: 2).PadLeft(32, '0')}\n";

                // Keys.A == 65 == 1000001
                int y = (int)m.LParam >> 16; // & 0xFFFF;
                label1.Text += $"(A1) {Convert.ToString(y, toBase: 2).PadLeft(32, '0')}\n";

                int y_2 = (int)m.LParam >> 16 & 0xFFFF;
                label1.Text += $"(A2) {Convert.ToString(y_2, toBase: 2).PadLeft(32, '0')}\n";

                int A = ((int)m.LParam >> 16) & 0xFFFF;
                Text = $"{A} == {Convert.ToString(A, toBase: 2).PadLeft(32, '0')}";
